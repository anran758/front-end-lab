...
TCP（传输控制协议）是一种可靠的、面向连接的协议，保证数据的正确传输和顺序到达，适用于需要高可靠性的应用。
而 UDP（用户数据报协议）是一种不可靠的、无连接的协议，传输速度快但不保证数据完整性，适用于对速度敏感或可以容忍数据丢失的场景。
...
HTTP 缓存策略可分**强缓存**与**协商缓存**。这些机制通过 HTTP 头部字段来控制，可以有效减少网络延迟，提高网页加载速度，并减轻服务器的负载。
...
### HTTP 2.0 和 HTTP 3.0 对比之前的版本, 分别做了哪些改进?

HTTP 每个版本的迭代都是为了解决现代网络需求下的性能瓶颈。

**HTTP/2 (相对于HTTP/1.1)**

1. **多路复用**：允许多个请求在单一的 TCP 连接上并行交互，消除了对多个连接的需求，并减少了延迟。
2. **服务器推送**：服务器可以未经请求主动向客户端发送资源，提高加载速度和用户体验。
3. **优先级和流控制**：提供请求的优先级设置，以及更细粒度的流量控制，允许服务器和客户端更有效地管理资源。
4. **头部压缩**：HTTP/2 引入了 HPACK 压缩格式来减少头部大小，降低了带宽使用。
5. **二进制格式**：HTTP/2 使用二进制而非文本格式来封装数据，提高了解析效率和性能。

**HTTP/3 (相对于HTTP/2)**

HTTP/3 最大的变化就是使用 QUIC 协议来代替 TCP 协议，QUIC 基于 UDP，提供了更快的连接建立和加密，以下是替换后的优势：

1. **改进的多路复用**：由于 QUIC 的改进，HTTP/3 的多路复用不受单一连接中的数据包丢失影响，降低了整体的延迟。
2. **连接迁移**：QUIC 支持连接 ID，即使用户的 IP 地址发生变化（例如，从 Wi-Fi 切换到移动网络），连接也可以保持不断开。
3. **更快的错误恢复**：QUIC 协议的设计使得它在检测到丢包时能够更快恢复，减少了重新传输所需的时间。
4. **内置TLS 1.3**：QUIC 协议包含了 TLS 1.3 加密，提供了更安全、更高效的数据加密。

![http2-stack-vs-http3-stack](./images/http2-stack-vs-http3-stack.png)

### HTTP 3.0 基于 udp 的话, 如何保证可靠的传输?

HTTP 3.0 准确来说是基于 QUIC 协议 (Quick UDP Internet Connections，快速 UDP 互联网连接)，而 QUIC 协议基于 UDP 协议。QUIC 协议通过以下关键机制实现：

1. **分组确认和重传**：通过确认收到的数据包并重传丢失的包，保证数据的完整性。
2. **流控制和拥塞控制**：限制发送速率以防止接收方被淹没，同时调整传输速度以应对网络条件。
3. **有序数据传输**：确保数据包即使乱序到达也能被正确重排。
4. **连接迁移**：使用唯一的连接ID允许即使在IP地址变化时也能保持连接的持续性。
