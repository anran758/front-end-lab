...
原文翻译[Airbnb JavaScript Style Guide](https://github.com/sivan/javascript-style-guide/blob/master/es5/README.md)  
[ECMA-6/ECMA2015](https://github.com/anran758/Front-End-Lab/Code-Guide/ECMA-6) - ECMAScript 6代码风格指南, 在原有风格的基础上作了修改和升级.

> 针对自己和团队的编程规范进行了一些修改, 以后会不定期的维护, 最后感谢开源者们的无私奉献.  
...
## 类型
  ...
  - [1.1](#1.1) <a name='1.1'></a> 原始值: 存取直接作用于它自身。
    ...
    ```
  - [1.2](#1.2) <a name='1.2'></a> 复杂类型: 存取时作用于它自身值的引用。
...
## 对象
  ...
  - [2.1](#2.1) <a name='2.1'></a>使用直接量创建对象。
  ...
  - [2.2](#2.2) <a name='2.2'></a>使用同义词替换需要使用的保留字。
...
## 数组
  ...
  - [3.1](#3.1) <a name='3.1'></a> 使用直接量创建数组。
  ...
  - [3.2](#3.2) <a name='3.2'></a> 当你需要拷贝数组时，使用 Array#slice。[jsPerf](http://jsperf.com/converting-arguments-to-an-array/7)
...
## 字符串
  ...
  - [4.1](#4.1) <a name='4.1'></a>使用单引号 `''` 包裹字符串。
  ...
  - [4.2](#4.2) <a name='4.2'></a> 程序化生成的字符串使用 Array#join 连接而不是使用连接符。尤其是 IE 下：[jsPerf](http://jsperf.com/string-vs-array-concat/2).
...
## 函数
 ...
  - 永远不要在一个非函数代码块（if、while 等）中声明一个函数，浏览器允许你这么做，但它们的解析表现不一致，正确的做法是：在块外定义一个变量，然后将函数赋值给它。
  ...
  - [5.2](#5.2) <a name='5.2'></a> 永远不要把参数命名为 `arguments`。这将取代函数作用域内的 `arguments` 对象。
